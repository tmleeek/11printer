 ...
 */
class Ooops_Catalog_Helper_Data extends Mage_Core_Helper_Abstract
{
            ...
            if ($category = $this->getCategory()) {
                ...
                foreach ($pathIds as $categoryId) {
                    if (isset($categories[$categoryId]) && $categories[$categoryId]->getName()) {
                        $catlev = Mage::getModel('catalog/category')->load($categoryId)->getLevel();
                        switch(strval($catlev)){
                            case '3':
                                $path['category'.$categoryId] = array(
                                    'label' => 'Выбор модели для принтеров '.$categories[$categoryId]->getName(),
                                    'link' => $this->_isCategoryLink($categoryId) ? $categories[$categoryId]->getUrl() : ''
                                );
                                break;
                            case '4':
                                $tmplb = $categories[$categoryId]->getName();
                                break;
                            case '5':
                                $path['category'.$categoryId] = array(
                                    'label' => $tmplb.' '.$categories[$categoryId]->getName(),
                                    'link' => $this->_isCategoryLink($categoryId) ? $categories[$categoryId]->getUrl() : ''
                                );
                                break;
                            default:
